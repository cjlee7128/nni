...
if __name__ == "__main__":
    ...
    for w1 in [8, 16, 24]: 
        for w2 in [16, 24, 32]: 
            width_stages = f'{w1},{w2}' 
            for c1 in [1, 2, 4]: 
                for c2 in [1, 2, 4]: 
                    n_cell_stages = f'{c1},{c2}' 
                    for s2 in [1, 2]: 
                        stride_stages = f'{1},{s2}'
                        for output_size in [8, 16, 24, 32, 40]: 
                            if i < 3: 
                                i = i + 1 
                                continue 
                            ...
                            cmd_str = ["python", "main.py", "--applied_hardware", "cortexA76cpu_tflite21", "--reference_latency", f"{reference_latency}", "--n_worker", f"{n_worker}", "--width_stages", f"{width_stages}", "--n_cell_stages", f'{n_cell_stages}', "--stride_stages", f'{stride_stages}', "--output_size", f'{output_size}', "--checkpoint_path", f'{PATH}', "--train_trainer_epochs", f"{train_trainer_epochs}"] 
                            subprocess.call(cmd_str, shell=False)
                            # retrain 
                            cmd_str = ["python", "main.py", "--applied_hardware", "cortexA76cpu_tflite21", "--reference_latency", f"{reference_latency}", "--n_worker", f"{n_worker}", "--train_mode", "retrain", "--width_stages", f"{width_stages}", "--n_cell_stages", f'{n_cell_stages}', "--stride_stages", f'{stride_stages}', "--output_size", f'{output_size}', "--checkpoint_path", f"{PATH}", "--exported_arch_path", f"{PATH}checkpoint.json", "--retrain_trainer_epochs", f"{retrain_trainer_epochs}"] 
                            subprocess.call(cmd_str, shell=False) 